...
class SelectionResult 
{
    ...
    constructor(aI:number, fI:number, cI:number)
    {
        this.anchorIndex = aI;
        this.focusIndex = fI;
...
export default class SyntaxHighlighter 
{
    ...
    constructor(bb:CodeBlock, codeInput:HTMLTextAreaElement, codeDisplay:HTMLDivElement)
    {
        this.bambooBlock = bb;
    ...
    updateDimensions()
    {
        ...
        this.codeDisplay.parentElement.style.height = finalHeight;
    ...
    finishStyling(parsedText:string)
    {
    ...
    getSelectionAndContent()
    {
        ...
        const selection = new SelectionResult(anchorIndex, focusIndex, currentIndex);
        const content = textSegments.map(({text}) => text).join('');

        return {selection, content};
    }

    restoreSelection(selection:SelectionResult)
    {
    ...
    {
        const textSegments = [];
        const childNodes = Array.from(parentNode.childNodes) as HTMLElement[];
    ...
    renderParsedText(parsedText : string)
    {
    ...
    renderParsedTextContentEditable(parsedText : string) 
    {
        if(!parsedText || parsedText == "") { return; }
